...
1. Archaeologist le as quatro dimensoes do Ikigai via
   ...
   trabalho da sessao.
2. Archaeologist abre a rodada de Continuous Discovery com **duas
...
3. Expert responde. Archaeologist escuta, registra, e repete por
   quantas rodadas forem necessarias ate o workflow ficar claro.
4. **Norte direcional (opcional).** Apos o workflow ancorar,
...
6. Archaeologist escreve `outcome-statement.yaml` na celula gerada
   ...
   passo 4, archaeologist adiciona o bloco `norte_direcional` ao
   YAML (formato igual ao de `smart-creator-os/outcome-statement.yaml`).
7. Archaeologist chama `ost-writer.writeRoot(celulaPath, outcome)` com
   o workflow ancorado. OST.md abre com a raiz preenchida.
8. Archaeologist gera handoff F1 para F2 via `handoff-engine.generate()`
   + `persist()` com o workflow, os ids e o caminho do OST. O handoff
   fica abaixo de 500 tokens por construcao.
9. Chief apresenta Playback Gate F1 em pt-BR. F1 e invariante critico:
